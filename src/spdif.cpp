...
void SPDIFOutput::set_output_active(bool active) {}

void SPDIFOutput::set_configs(SPDIFConfigs cfg) {
    _cfg = cfg;
    ...
    for (int i = 2; i < SPDIF_BUF_ARRAY_SIZE; i += 2) {
        _spdif_buf[i] = bmc_mw ^= BMC_MW_DIF;
    }

    _spdif_current_ptr = _spdif_buf;
}

bool SPDIFOutput::begin() {
    if (_cfg.pin == GPIO_NUM_NC) return false;

    _ring_buffer = xRingbufferCreate(SPDIF_BUF_SIZE * 16, RINGBUF_TYPE_BYTEBUF);
    xTaskCreatePinnedToCore(
            [](void *context) { reinterpret_cast<SPDIFOutput *>(context)->task(); },
            "SPDIFOutput",
            _cfg.task_stack_size,
            this,
            _cfg.task_priority,
            _spdif_task,
            tskNO_AFFINITY
...
size_t SPDIFOutput::write(const uint8_t *data, size_t len) {
    for (int i = 0; i < len; i += 2) {
        /**
             ...
             */
        uint32_t lo = bmc_convert[data[i]] << 16;
        uint32_t hi = bmc_convert[data[i + 1]];

        *(_spdif_current_ptr + 1) = ((lo ^ hi) << 1) >> 1;

        _spdif_current_ptr += 2;  // advance to next audio data

        if (_spdif_current_ptr >= &_spdif_buf[SPDIF_BUF_ARRAY_SIZE]) {
            xRingbufferSend(_ring_buffer, _spdif_buf, sizeof(_spdif_buf), portMAX_DELAY);
            _spdif_current_ptr = _spdif_buf;
        }
    }
    return len;
...
void SPDIFOutput::end() {
    vTaskDelete(_spdif_task);
    vRingbufferDelete(_ring_buffer);
    i2s_driver_uninstall(_cfg.i2s_port);
}

void SPDIFOutput::set_sample_rate(int rate) {
    rate *= 2;
    int bclk = rate * 64 * 2;
    ...
    i2s_pin_config_t pin_config = {
            .bck_io_num = -1,
            .ws_io_num = -1,
            .data_out_num = _cfg.pin,
            .data_in_num = -1,
    };
    i2s_driver_install(_cfg.i2s_port, &i2s_config, 0, nullptr);
    i2s_set_pin(_cfg.i2s_port, &pin_config);
}


void SPDIFOutput::task() {
    size_t size;
    while (true) {
        auto item_ptr = xRingbufferReceiveUpTo(
                _ring_buffer,
                &size,
                portMAX_DELAY,
                512
        ...
        size_t written = 0;
        while (written < size) {
            i2s_write(_cfg.i2s_port, item_ptr, size, &written, portMAX_DELAY);
        }
        vRingbufferReturnItem(_ring_buffer, item_ptr);
    }
}
